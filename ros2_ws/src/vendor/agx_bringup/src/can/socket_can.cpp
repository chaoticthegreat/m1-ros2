...
namespace agx::nav
{
    ...
    SocketCan::SocketCan(rclcpp::Node* node) 
        : m_node(node),
          ...
          m_initialized(false)
    {
        // [M1 vendor patch] Honor the `interface` node parameter (default "can0")
        // instead of the previously-hardcoded interface, so the base CAN bus can
        // be selected at launch (e.g. "can1" when the Damiao arm bus owns can0).
        // Upstream agx_bringup declares `interface` in its yaml but never reads it;
        // this wires it through before init_can() opens the socket below.
        if (m_node)
        {
            if (!m_node->has_parameter("interface")) {
                m_node->declare_parameter<std::string>("interface", m_interface);
            }
            m_node->get_parameter("interface", m_interface);
        }
